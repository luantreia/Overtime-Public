...
import type { SolicitudEdicion, SolicitudEdicionEstado } from '../../../types/solicitudesEdicion';
...
export const NotificacionesPanel: React.FC<NotificacionesPanelProps> = ({
  ...
  const cargar = useCallback(async () => {
    try {
      ...
      // Filtrar por tipos permitidos (si se especificaron)
      const allowedSet = allowedTipos ? new Set(allowedTipos) : null;
      const filtradas = allowedSet
        ? data.solicitudes.filter((s: SolicitudEdicion) => allowedSet.has(s.tipo as any))
        : data.solicitudes;
  ...
  const filtradas = useMemo(() => {
    return solicitudes.filter((s: SolicitudEdicion) => {
      // Filtro extra (ej: asociación con un jugador específico)
  ...
  }, [solicitudes, extraFilter, fCategoria, q, categoriaDeTipo, labelTipo]);
  ...
  const manejarAprobarSeleccionadas = async () => {
    ...
    try {
      ...
      const actualizadas = new Map<string, SolicitudEdicion>();
