          ...
          {solicitudesPendientes.map((solicitud) => (
            <li key={solicitud.id} className="flex flex-wrap items-center gap-3 rounded-xl bg-white px-4 py-3 text-sm shadow-sm">
              <div className="flex-1">
                <p className="font-semibold text-slate-900">{solicitud.jugador?.nombre ?? 'Jugador desconocido'}</p>
                <p className="text-xs text-slate-500">
                  Posición: {solicitud.jugador?.posicion ?? 'N/A'} • Estado: {solicitud.estado}
                </p>
                {solicitud.mensaje ? (
