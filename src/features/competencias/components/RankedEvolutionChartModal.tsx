    ...
    >
      ...
      >
        <div className="px-6 py-5 flex items-center justify-between bg-white shrink-0">
          <div>
            <h3 className="text-xl font-extrabold text-slate-800 tracking-tight">Evoluci�n de Ranking</h3>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col min-h-0">
          <div className="px-6 pb-4 bg-white flex flex-col gap-4 shrink-0">
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-0.5">
              {[{ id: "all", label: "Temporada" }, { id: "month", label: "�ltimo Mes" }].map((f) => (
                <button
                  key={f.id}
            ...
            <div className="flex items-center justify-between gap-3 bg-slate-50/80 p-1.5 rounded-2xl border border-slate-100">
               <div className="flex items-center gap-1">
                  <button onClick={() => setViewType("line")} className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase transition-all ${viewType === "line" ? "bg-white text-brand-600 shadow-sm" : "text-slate-400"}`}>L�neas</button>
                  <button onClick={() => setViewType("area")} className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase transition-all ${viewType === "area" ? "bg-white text-brand-600 shadow-sm" : "text-slate-400"}`}>�reas</button>
               </div>
          ...
          <div className="flex-1 w-full flex flex-col min-h-[400px] bg-white px-2 sm:px-6 pb-6 overflow-hidden">
            {isLoading ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-slate-50/50 rounded-[32px] mb-4">
                <h4 className="text-slate-800 font-black text-sm uppercase tracking-wider">Historial vac�o</h4>
              </div>
            )}
          </div>
