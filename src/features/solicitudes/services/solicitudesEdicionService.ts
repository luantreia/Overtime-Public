...
export const getSolicitudesEdicion = async (
  ...
  try {
    const response = await authFetch<SolicitudesPaginadas>(`/solicitudes-edicion?${params.toString()}`);
    return response;
  } catch (error: any) {
    if (error.status === 400) {
 ...
 */
export const getSolicitudEdicionById = async (id: string): Promise<SolicitudEdicion> => {
  try {
    return await authFetch<SolicitudEdicion>(`/solicitudes-edicion/${id}`);
  } catch (error: any) {
    if (error.status === 404) {
...
export const getSolicitudOpciones = async (
  ...
  tiposDisponibles: SolicitudOpciones[];
}> => {
  const params = new URLSearchParams();
...
export const crearSolicitudEdicion = async (
  payload: SolicitudCrearPayload
): Promise<SolicitudEdicion> => {
  try {
    return await authFetch<SolicitudEdicion>('/solicitudes-edicion', {
      method: 'POST',
...
export const actualizarSolicitudEdicion = async (
  id: string,
  payload: SolicitudActualizarPayload
): Promise<SolicitudEdicion> => {
  try {
    return await authFetch<SolicitudEdicion>(`/solicitudes-edicion/${id}`, {
      method: 'PUT',
 ...
 */
export const getSolicitudAprobadores = async (id: string): Promise<SolicitudAprobadores> => {
  try {
    return await authFetch<SolicitudAprobadores>(`/solicitudes-edicion/${id}/aprobadores`);
  } catch (error: any) {
    if (error.status === 404) {
      throw new Error('Solicitud no encontrada');
    }
