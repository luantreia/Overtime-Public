...
const NotificacionesPage = () => {
  ...
  const perteneceAlJugador = (s: SolicitudEdicion, jugadorId: string) => {
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      {!jugadorSeleccionado ? (
        ...
        (Object.entries(
          filtradas.reduce((acc: Record<string, SolicitudEdicion[]>, s: SolicitudEdicion) => {
            const cat = categoriaDeTipo((s as any).tipo);
            if (!acc[cat]) acc[cat] = [];
            acc[cat].push(s);
            return acc;
          }, {}) as Record<string, SolicitudEdicion[]>
        ) as [string, SolicitudEdicion[]][]).map(([cat, items]) => (
          <section key={cat} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {(items as SolicitudEdicion[]).slice((page - 1) * pageSize, (page - 1) * pageSize + pageSize).map((s: any) => (
                    <>
